...
// @description    Barra con funciones mamonas para el Monstro, ahora con moas weas!!
...
defaultCfg = {
	...
	'findImgFile':	1,		// 		detect built-in files in images
	...
	'linksOver':	40,	//		delay appearance in ms
	'linksOut':		30,	//		delay disappearance in ms
	...
	'ajaxReply':	0,		// posting with AJAX (0=no, 1=iframe, 2=HTML5)
	'postSameImg':	0,		// 		ability to post same images
	'removeEXIF':	0,		// 		remove EXIF data from JPEGs
	'removeFName':	0,		// 		remove file name
	'addPostForm':	0,		// postform displayed [0=at top, 1=at bottom, 2=inline, 3=hanging]
	'scrAfterRep':	1,		// scroll to the bottom after reply
	'noThrdForm':	0,		// hide thread-creating form
	'favOnReply':	0,		// add thread to favorites on reply
	'addSageBtn':	0,		// email field -> sage btn
	'warnSubjTrip':	0,		// warn if subject field contains tripcode
	'saveSage':		0,		//		remember sage
	'sageReply':	0,		//		reply with sage
	'captchaLang':	0,		// language input in captcha [0=off, 1=en, 2=ru]
	'addTextBtns':	0,		// text format buttons [0=off, 1=graphics, 2=text, 3=usual]
	...
	'noGoto':		0,		// hide goto field
	'noPassword':	0,		// hide password field
	...
	'panelCounter':	0,		// posts/images counter in script panel
	...
	'scrUpdIntrv':	0,		// 		check interval in days (every val+1 day)
...
Lng = {
	cfg: {
		'hideBySpell':	['Заклинания: ', 'Comandos personalizados: '],
	...
	txtBtn: {
		'bold':		['Жирный', 'Negritas'],
		'italic':	['Наклонный', 'Cursiva'],
		'under':	['Подчеркнутый', 'Subrayado'],
		'strike':	['Зачеркнутый', 'Tachado'],
		'spoil':	['Спойлер', 'Spoiler'],
		'code':		['Код', 'Encabezado'],
		'quote':	['Цитировать выделенное', 'Implicar (protip: si seleccionas primero el texto, lo añadirá a tu respuesta :3)']
	},
	...
	panelBtn: {
		'attach':	['Прикрепить/Открепить', 'Fijar/Auto-ocultar'],
		'settings':	['Настройки', 'Desactivado ;____;'],
		'hidden':	['Скрытое', 'mierda hideada'],
		'favor':	['Избранное', 'Favoritos'],
		'refresh':	['Обновить', 'Re/cargar página(s)'],
		'goback':	['Назад', 'Atrás'],
		'gonext':	['Следующая', 'Adelante'],
		'goup':		['Наверх', 'Ir hasta arriba'],
		'godown':	['В конец', 'Ir hasta abajo'],
		'newthr':	['Создать тред', 'Hilo...'],
		'expimg':	['Раскрыть картинки', 'Expandir imágenes'],
		'maskimg':	['Маскировать картинки', 'Modo SWF'],
		'upd-on':	['Автообновление треда', 'Thread autoupdate'],
		'audio-off':['Звуковое оповещение о новых постах', 'Notificacion de audio'],
		'catalog':	['Каталог', 'Catálogo'],
		'counter':	['Постов/Изображений в треде', 'Posts/Imágenes EEH'],
		'imgload':	['Сохранить изображения из треда', 'Descargar imágenes...']
	},
	...
	selAjaxPages:	[
		...
		['Solo página 0', 'Páginas 0 y 1', 'Páginas 0, 1 y 2', 'Pp. 0-3', 'Pp- 0-4']
	],
	selAudioNotif:	[
		['Каждые 30 сек.', 'Каждую минуту', 'Каждые 2 мин.', 'Каждые 5 мин.'],
		['Cada 30 sec.', 'Cada minuto', 'Cada 2 min.', 'Cada 5 min.']
	],
	...
	editor:			{
		...
		hidden:		['Редактирование скрытых тредов:', 'Etiquetar hilos poendejos'],
		favor:		['Редактирование избранного:', 'Editar favoritos:'],
	...
	add:			['Добавить', 'Añadir'],
	apply:			['Применить', 'Aplicar'],
	clear:			['Очистить', 'Limpiar'],
	help:			['Помощь', 'Ayuda'],
	refresh:		['Обновить', 'Recargar'],
	load:			['Загрузить', 'Subir'],
	save:			['Сохранить', 'Guardar'],
	edit:			['Правка', 'Editar'],
	reset:			['Сброс', 'Reset'],
	remove:			['Удалить', 'Remover'],
	info:			['Инфо', 'Pls respond'],
	undo:			['Отмена', 'Deshacer'],
	loading:		['Загрузка...', 'Ahi va...'],
	checking:		['Проверка...', 'Actualizando... '],
	deleting:		['Удаление...', 'Y nada de valor se perderá...'],
	error:			['Ошибка:', 'Error:'],
	noConnect:		['Ошибка подключения', 'Pinche internet culero'],
	thrNotFound:	['Тред недоступен (№', 'No se encontró la wea (№'],
	succDeleted:	['Успешно удалено!', 'Y nada de valor se perdió!'],
	...
	editInTxt:		['Правка в текстовом формате', 'Especifica una etiqueta'],
	...
	clrSelected:	['Удалить выделенные записи', 'Quitar la wea seleccionada'],
	saveChanges:	['Сохранить внесенные изменения', 'Guardar los cambios'],
	infoCount:		['Обновить счетчики постов', 'Comprobar si hay nuevas respuestas'],
	infoPage:		['Проверить актуальность тредов (до 5 страницы)', 'Buscar en cuál página está el favorito'],
	clrDeleted:		['Очистить записи недоступных тредов', 'Limpiar lolcache'],
	hiddenPosts:	['Скрытые посты на странице', 'Posts mierdosos'],
	hiddenThrds:	['Скрытые треды', 'Hilos mierdosos'],
	noHidPosts:		['На этой странице нет скрытых постов...', 'No hay ni madres de posts...'],
	noHidThrds:		['Нет скрытых тредов...', 'No hay ni madres...'],
	expandAll:		['Раскрыть все', 'Expandir todo'],
	invalidData:	['Некорректный формат данных', 'Incorrect data format'],
	favThrds:		['Избранные треды:', 'Favoritos:'],
	noFavThrds:		['Нет избранных тредов...', 'No estás mamando a nadie...'],
	reply:			['Ответ', 'Pls Respond'],
	replyTo:		['Ответ в', 'Responder a'],
	replies:		['Ответы:', 'Respuestas:'],
	postsOmitted:	['Пропущено ответов: ', 'Posts omitidos: '],
	collapseThrd:	['Свернуть тред', 'Contraer Hilo'],
	deleted:		['удалён', 'borrado?'],
	getNewPosts:	['Получить новые посты', 'Actualizar'],
	page:			['Страница', 'Página'],
	hiddenThrd:		['Скрытый тред:', 'Hilo mierdero:'],
	expandForm:		['Раскрыть форму', 'Expandir formulario'],
	search:			['Искать в ', 'Buscar... '],
	wait:			['Ждите', 'Espere'],
	addFile:		['+ файл', 'Incrustar ChenPe'],
	helpAddFile:	['Добавить .ogg, .rar, .zip, или .7z к картинке', 'Esta mierda solo sirve en sus chanes rusos chingones'],
	downloadFile:	['Скачать содержащийся в картинке файл', 'Descargar el archivo incrustado que no es CP'],
...
//	gif('#de-btn-logo', p + 'I5jI+pywEPWoIIRomz3tN6K30ixZXM+HCgtjpk1rbmTNc0erHvLOt4vvj1KqnD8FQ0HIPCpbIJtB0KADs=');
	...
	x += '.de-ppanel { margin-left: 4px; }\
		...
		.de-btn-hide, .de-btn-hide-user, .de-btn-rep, .de-btn-fav, .de-btn-fav-sel, .de-btn-src, .de-btn-expthr{ display: inline-block; margin: 0 4px -2px 0 !important; cursor: pointer; ';
	if(!Cfg['postBtnsTxt']) {
